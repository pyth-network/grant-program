...
export function useSolanaSignMessage(): SignMessageFn {
  const { connected, signMessage, publicKey, signTransaction } =
    useSolanaWallet()
  const signMessageCb = useCallback(
    async (payload: string) => {
      try {
        ...
        console.log('HANDROLLED :', signBuffer.toString('hex'))
        console.log('BACKPACK : ', Buffer.from(messagePayload).toString('hex'))
        console.log(
          'EQUALS : ',
          signBuffer.toString('hex') ==
            Buffer.from(messagePayload).toString('hex')
        )
