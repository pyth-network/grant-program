use {
    super::test_evm::construct_evm_pubkey,
    crate::{
        ecosystems::{
            cosmos::{
                CosmosMessage,
...
impl InjectiveTestIdentityCertificate {
    ...
    pub fn random(claimant: &Pubkey) -> Self {
        ...
        let (signature, recovery_id) =
            libsecp256k1::sign(&prefixed_message.hash_injective(), &secret);
        Self {
...
impl From<InjectiveTestIdentityCertificate> for Identity {
    fn from(val: InjectiveTestIdentityCertificate) -> Self {
        Identity::Injective {
            address: val.recover_as_evm_address().into(),
        }
    }
}
